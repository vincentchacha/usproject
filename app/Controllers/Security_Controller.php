...
class Security_Controller extends App_Controller {
    ...
    protected function get_access_info($group) {
        ...
        if ($this->login_user->is_admin||$this->login_user->user_type=="sponsor") {
    ...
    protected function can_create_projects() {
        if ($this->login_user->user_type == "staff"||$this->login_user->user_type == "sponsor") {
            if ($this->login_user->is_admin || get_array_value($this->login_user->permissions, "can_manage_all_projects") == "1"||$this->login_user->user_type == "sponsor") {
                return true;
    ...
    protected function init_project_permission_checker($project_id = 0) {
        if ($this->login_user->user_type == "client"||$this->login_user->user_type == "sponsor") {
            $project_info = $this->Projects_model->get_one($project_id);
            if ($project_info->client_id == $this->login_user->client_id||$this->login_user->user_type == "sponsor") {
    ...
    protected function can_manage_all_projects() {
        if ($this->login_user->is_admin ||$this->login_user->user_type == "sponsor"|| get_array_value($this->login_user->permissions, "can_manage_all_projects") == "1") {
            return true;
        }
    }
